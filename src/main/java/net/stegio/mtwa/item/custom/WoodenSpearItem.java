...
public class WoodenSpearItem extends Item implements ProjectileItem {
                ...
                if (!(f > 0.0F) || playerEntity.isTouchingWaterOrRain()) {
                    if (!isAboutToBreak(stack)) {
                        ...
                        if (!world.isClient) {
                            ...
                            if (f == 0.0F) {
                                WoodenSpearProjectileEntity wooden_spear_entity = new WoodenSpearProjectileEntity(world, playerEntity, stack);
                                wooden_spear_entity.setVelocity(playerEntity, playerEntity.getPitch(), playerEntity.getYaw(), 0.0F, 2.5F, 1.0F);
                                if (playerEntity.isInCreativeMode()) {
                                    wooden_spear_entity.pickupType = PersistentProjectileEntity.PickupPermission.CREATIVE_ONLY;
                                }

                                world.spawnEntity(wooden_spear_entity);
                                world.playSoundFromEntity(null, wooden_spear_entity, registryEntry.value(), SoundCategory.PLAYERS, 1.0F, 1.0F);
                                if (!playerEntity.isInCreativeMode()) {
    ...
    public ProjectileEntity createEntity(World world, Position pos, ItemStack stack, Direction direction) {
        WoodenSpearProjectileEntity wooden_spear_entity = new WoodenSpearProjectileEntity(world, pos.getX(), pos.getY(), pos.getZ(), stack.copyWithCount(1));
        wooden_spear_entity.pickupType = PersistentProjectileEntity.PickupPermission.ALLOWED;
        return wooden_spear_entity;
    }
}
